...
Get_Env_Data_A <- function(path,source_path,date_range){
  ...
  envdir=glue("{path}/daily_prediction_layers");dir.create(envdir)
  outdir <- glue("{path}/end_products_final");dir.create(outdir)
  staticdir=glue("{path}/static_variables");dir.create(staticdir)
  # temp=glue("{path}/raw_roms_data")
  moddir=glue("{path}/models");dir.create(moddir)
  # intermediatedir=glue("{path}/intermediate")
  
  # flyersdir=glue("{outdir}/flyers")
  rastersdir=glue("{outdir}/rasters");dir.create(rastersdir)
  mapssdir=glue("{outdir}/maps");dir.create(mapssdir)
  epac_metadatadir=glue("{outdir}/epac_metadata")
  ramp_zone_indicatorsdir=glue("{outdir}/ramp_zone_indicators");dir.create(ramp_zone_indicatorsdir)
  two_week_mapsdir=glue("{outdir}/two_week_maps");dir.create(two_week_mapsdir)
  two_week_metadatadir=glue("{outdir}/two_week_metadata");dir.create(two_week_metadatadir)
  two_week_rastersdir=glue("{outdir}/two_week_rasters");dir.create(two_week_rastersdir)
