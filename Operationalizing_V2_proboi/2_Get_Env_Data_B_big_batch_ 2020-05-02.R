...
Get_Env_Data_B_batch=function(path,source_path,date_range){
  ...
  handle_the_data=function(get_date,var,template,save_var,finaldir){ 
    ...
    if(var=="ild_05"){
      ## new stuff for NAs
      a=rast(r2)
      b=terra::focal(a,w=3,fun="mean",na.policy="only")
      b_ras=raster(b)
      ##
      
      r3_hump <- raster::resample(b_ras, template_humpback,method="ngb")  
    ...
    if(var=="ssh"){
      ...
      r3_hump <- raster::resample(b_ras, template_humpback,method="ngb") + 0.035
      extent(r3_hump)=extent(template_humpback)
      # return(r3)
      writeRaster(r3_hump,glue("{finaldir}/{save_var}_humpback.grd"),overwrite=T)
    }
