...
predict_bluewhale <- function(path,source_path,date_range){
  ...
  if(!file.exists(glue("{rastersdir}/bluewhale_{get_date}.grd"))){
  ...
  tryCatch(
    ...
    writeRaster(meanPredR,glue("{rastersdir}/bluewhale_{get_date}.grd"),overwrite=T)
    ...
    png(glue("{mapssdir}/bluewhale_{get_date}.png"),width=8,height=11,units='cm',res=400,type = "cairo")
    par(ps=10)
    par(mar=c(0,0,0,0))
    par(cex=1)
    print({pred_map})
