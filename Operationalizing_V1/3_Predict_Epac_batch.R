...
predict_epac <- function(path,source_path,date_range){
  ...
  if(file.exists(glue("{outdir}/epac_metadata/metadata_{get_date}.csv"))){
  metadata=read.csv(glue("{outdir}/epac_metadata/metadata_{get_date}.csv")) %>% 
  ...
  tryCatch(
    ...
    write.csv(metadata,glue("{outdir}/epac_metadata/metadata_{get_date}.csv"))
    ...
    writeRaster(pred_ras_clip,glue("{rastersdir}/epac_{get_date}.grd"),overwrite=T)
    ...
    png(glue("{mapssdir}/epac_{get_date}.png"),width=8,height=11,units='cm',res=400,type = "cairo")
    par(ps=10)
    par(mar=c(0,0,0,0))
    par(cex=1)
    print({pred_map})
